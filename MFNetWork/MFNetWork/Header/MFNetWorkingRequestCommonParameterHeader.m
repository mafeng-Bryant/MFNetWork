//
//  MFNetWorkingRequestCommonParameterHeader.m
//  MFNetWork
//
...
@implementation MFNetWorkingRequestCommonParameterHeader

+ (NSDictionary *)commonHeaders;
{
    NSString *currencyCode = [[NSLocale currentLocale] objectForKey:NSLocaleCurrencyCode];
    NSString *selectCurrency = [PPSetting sharedPPSetting].currencyHandler.currencyISO;
    if ([PPSetting sharedPPSetting].currencyHandler.isSysDefault) {
        currencyCode = selectCurrency;
    }
    return @{@"appversion":kAppVersion,
             @"buildversion":kAppBuildVersion,
             @"platform":@"ios",
             @"screensize":FormatString([MDevice screenSize],@""),
             @"country":FormatString([[NSLocale currentLocale] objectForKey:NSLocaleCountryCode],@""),
             @"language":FormatString([PPLocalizationManager shareLocalizable].normalLocalizable,@""),
             @"timezonename":FormatString([MDevice timeZone],@""),
             @"timezone":FormatString([NSTimeZone systemTimeZone].abbreviation,@""),
             @"deviceid":FormatString([UIDevice deviceUUID],@""),
             @"currency":FormatString([[MDevice currency] stringByAddingPercentEncodingWithAllowedCharacters:[NSCharacterSet URLQueryAllowedCharacterSet]],@""),
             @"countrycurrency":FormatString(currencyCode,@"")};
}
